...
class MaskCleanConfig:
    """
    پارامترهای قابل‌تنظیمِ سیستم ماسک/پاک‌سازی (adaptive padding + cluster cleaning).

    همه‌ی مقادیر پیش‌فرض طوری انتخاب شده‌اند که روی یک صفحه‌ی معمولیِ ~۱۰۰۰px
    رفتاری نزدیک به رفتار قبلی پروژه بدهند، ولی با اندازه‌ی واقعی متن سازگار شوند.
    برگشت کامل به رفتار قدیمی: adaptive_mask=False و cluster_clean=False
    (CLI: --no-adaptive-mask --no-cluster-clean).
    """
    # ── کلیدهای اصلی ─────────────────────────────────────────────
    adaptive_mask: bool = True      # padding و باند halo بر اساس اندازه‌ی واقعی متن
    cluster_clean: bool = True      # پاک‌سازی cluster به cluster با context محلی

    # ── padding تطبیقی (px) ─ مقدار پایه همان mask_padding است (پیش‌فرض ۳) ──
    pad_min: int = 2                # متن کوچک ≈ 2–3
    textured_keeps_base_pad: bool = True  # روی زمینه‌ی بافت‌دار، padding زیر mask_padding نمی‌رود (flat-fill نمی‌تواند لبه را جبران کند)
    pad_max: int = 6                # متن بزرگ ≈ 4–6
    ref_side: int = 1000            # ضلع مرجع برای اثر resolution (min(H, W))

    # ── halo رنگی ────────────────────────────────────────────────
    halo_band_ratio: float = 0.22   # پهنای باند جست‌وجوی halo = نسبت × ارتفاع متن (≈۴px برای متن ۲۰px)
    halo_guard: bool = True         # اگر خودِ زمینه بنفش/رنگی بود، فقط پیکسل‌های متمایز از زمینه
    halo_purple_bg_frac: float = 0.35
    halo_bg_delta: float = 12.0     # حداقل فاصله‌ی رنگی (Lab-8bit) از زمینه در حالت guard
    halo_any_hue: bool = False      # (اختیاری) halo هر رنگی روی زمینه‌ی یک‌دست، نه فقط بنفش
    halo_chroma_delta: float = 14.0

    # ── cluster و context ───────────────────────────────────────
    cluster_gap_ratio: float = 0.8  # فاصله‌ی ادغام = نسبت × ارتفاع متن
    cluster_gap_min: int = 6
    cluster_gap_max: int = 40
    context_ratio: float = 0.5      # context = نسبت × بُعد بزرگ‌ترِ cluster
    context_min: int = 24           # px (در resolution مرجع)
    context_max: int = 96           # px (در resolution مرجع)

    # ── flat-fill ────────────────────────────────────────────────
    flat_std_thresh: float = 13.0   # همان آستانه‌ی قبلی
    flat_min_ring_px: int = 40
    flat_robust: bool = True        # تحمل چند پیکسل دیوار/لبه در حلقه‌ی دور (نه نزدیکِ ماسک)
    flat_inlier_tol: float = 20.0
    flat_inlier_frac: float = 0.88

    def __post_init__(self) -> None:
        self.pad_min = max(0, int(self.pad_min))
        self.pad_max = max(self.pad_min, int(self.pad_max))
        self.context_min = max(8, int(self.context_min))
        self.context_max = max(self.context_min, int(self.context_max))
        self.cluster_gap_min = max(1, int(self.cluster_gap_min))
        self.cluster_gap_max = max(self.cluster_gap_min, int(self.cluster_gap_max))

    def apply_overrides(self, items) -> "MaskCleanConfig":
        """items: لیستی از رشته‌های KEY=VALUE (مثلاً از --mask-opt). کلید ناشناخته → ValueError."""
        import dataclasses
        types = {f.name: f.type for f in dataclasses.fields(self)}
        for raw in (items or []):
            if "=" not in raw:
                raise ValueError(f"فرمت نادرست (باید KEY=VALUE باشد): {raw!r}")
            key, val = (t.strip() for t in raw.split("=", 1))
            if key not in types:
                raise ValueError(f"پارامتر ناشناخته: {key!r} (مجاز: {', '.join(sorted(types))})")
            t = str(types[key])
            if "bool" in t:
                setattr(self, key, val.lower() in ("1", "true", "yes", "on"))
            elif "int" in t:
...
class MangaTranslator:
    ...
    def __init__(
        ...
        mask_cfg: Optional["MaskCleanConfig"] = None,
    ):
        ...
        self.mask_cfg = mask_cfg if mask_cfg is not None else MaskCleanConfig()
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  ماسک تطبیقی + پاک‌سازی cluster-به‌cluster
    #  ترتیب: OCR polygon → refine → halo رنگی → padding تطبیقی → cluster
    #         → flat-fill → LaMa با context محلی → OpenCV fallback
    #  هر دو کلید adaptive_mask و cluster_clean در MaskCleanConfig قابل خاموش شدن‌اند
    #  و خاموش‌کردنشان دقیقاً به کد قدیمی (…_legacy / _clean_page_level) برمی‌گردد.
    # ══════════════════════════════════════════════════════════════════════

    _PURPLE_LO = np.array([125, 40, 40], dtype=np.uint8)
    _PURPLE_HI = np.array([160, 255, 255], dtype=np.uint8)
    # نگاشت «ارتفاع واقعی متن (px)» → padding پایه (px)؛ کوچک≈۲–۳، متوسط≈۳–۴، بزرگ≈۴–۶
    _PAD_ANCHOR_H = (8.0, 14.0, 20.0, 30.0, 45.0, 70.0)
    _PAD_ANCHOR_P = (2.0, 2.5, 3.0, 4.0, 5.0, 6.0)
    _NOMINAL_TEXT_H = 20.0      # وقتی اندازه‌ی متن معلوم نیست (region بدون ocr_polys)
    _PROMO_R = 21               # شعاع اثر dilation قدیمیِ promo/sfx (کرنل ۱۵ × ۳ بار)
    _MIN_CROP_SIDE = 64         # حداقل ضلع crop برای inpaint

    def _mcfg(self) -> "MaskCleanConfig":
        cfg = getattr(self, "mask_cfg", None)
        if cfg is None:
            cfg = MaskCleanConfig()
            self.mask_cfg = cfg
        return cfg

    def _res_scale(self, h: int, w: int, exponent: float, lo: float, hi: float) -> float:
        ref = float(max(1, self._mcfg().ref_side))
        r = max(1.0, float(min(h, w))) / ref
        return float(min(hi, max(lo, r ** exponent)))

    @staticmethod
    def _poly_text_height(pts_list) -> float:
        """ارتفاع واقعی متن = میانه‌ی ضلع کوتاهِ minAreaRect هر خطِ OCR (برای متن افقی و عمودی)."""
        hs = []
        for pts in pts_list:
    ...
    def _adaptive_pad_px(self, text_h: float, res_f: float) -> int:
        base = float(max(0, int(getattr(self, "mask_padding", 3) or 3)))
        if not (text_h > 0):
            return int(base)            # اندازه نامعلوم → همان مقدار قبلیِ پروژه
        cfg = self._mcfg()
        p = float(np.interp(text_h, self._PAD_ANCHOR_H, self._PAD_ANCHOR_P)) * res_f * (base / 3.0)
        p = min(float(cfg.pad_max), max(float(cfg.pad_min), p))
        return int(p + 0.5)

    def _halo_band_px(self, text_h: float) -> int:
        if not (text_h > 0):
            return 4                    # معادل کرنل ۹×۹ قبلی
        return int(min(8, max(2, round(self._mcfg().halo_band_ratio * text_h))))

    def _halo_candidates(self, roi_img: np.ndarray, m0: np.ndarray, band: int) -> Optional[np.ndarray]:
        """
        پیکسل‌های halo در باند اطراف ماسک (فقط پیکسل‌های جدید، خارج از m0).
        - آشکارساز بنفش: همان آستانه‌های قبلی پروژه.
        - guard زمینه: اگر خودِ زمینه بنفش است (یا تقریباً کل باند «بنفش» شد)، فقط پیکسل‌هایی
          که از رنگ زمینه فاصله دارند نگه داشته می‌شوند تا bubble/زمینه‌ی سالم ماسک نشود.
        - halo_any_hue (اختیاری): پیکسل‌های رنگیِ هر فام، فقط روی زمینه‌ی یک‌دست.
        """
        cfg = self._mcfg()
        hsv = cv2.cvtColor(roi_img, cv2.COLOR_BGR2HSV)
        purple = cv2.inRange(hsv, self._PURPLE_LO, self._PURPLE_HI)
        near = cv2.dilate(m0, np.ones((2 * band + 1, 2 * band + 1), np.uint8))
        zone = (near > 0) & (m0 == 0)
        cand = (purple > 0) & zone
        if not cand.any() and not cfg.halo_any_hue:
            return None

        need_ring = cfg.halo_guard or cfg.halo_any_hue
        ring = None
        lab = None
        bg = None
        if need_ring:
            inner = cv2.dilate(m0, np.ones((2 * band + 3, 2 * band + 3), np.uint8)) > 0
            outer = cv2.dilate(m0, np.ones((2 * (band + 7) + 1, 2 * (band + 7) + 1), np.uint8)) > 0
            ring = outer & ~inner
            if int(np.count_nonzero(ring)) < 30:
                ring = None

        if cfg.halo_guard and ring is not None and cand.any():
            ring_n = int(np.count_nonzero(ring))
            purple_frac = float(np.count_nonzero(purple[ring])) / ring_n
            zone_n = int(np.count_nonzero(zone))
            cov = float(np.count_nonzero(cand)) / max(1, zone_n)
            strict = purple_frac >= cfg.halo_purple_bg_frac or (zone_n >= 40 and cov >= 0.85)
            if strict:
                lab = cv2.cvtColor(roi_img, cv2.COLOR_BGR2LAB).astype(np.float32)
                bg = np.median(lab[ring], axis=0)
                d = np.linalg.norm(lab - bg, axis=2)
                cand = cand & (d > cfg.halo_bg_delta)

        if cfg.halo_any_hue and ring is not None:
            if lab is None:
                lab = cv2.cvtColor(roi_img, cv2.COLOR_BGR2LAB).astype(np.float32)
            ring_lab = lab[ring]
            if float(ring_lab.std(axis=0).max()) <= 6.0:
                bg_ab = np.median(ring_lab, axis=0)[1:3]
                dev = np.linalg.norm(lab[..., 1:3] - bg_ab, axis=2)
                cand = cand | (zone & (dev > cfg.halo_chroma_delta))

        if not cand.any():
            return None
        return cand.astype(np.uint8) * 255

    def _bg_is_textured(self, roi_img: np.ndarray, mask: np.ndarray) -> bool:
        """آیا زمینه‌ی اطراف ماسک یک‌دست نیست؟ (همان معیار std حلقه‌ی flat-fill). ناکافی بودن نمونه → محتاطانه True."""
        cfg = self._mcfg()
        ring = (cv2.dilate(mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (21, 21))) > 0) & (mask == 0)
        if int(np.count_nonzero(ring)) < cfg.flat_min_ring_px:
            return True
        return float(roi_img[ring].astype(np.float32).std(axis=0).max()) > cfg.flat_std_thresh

    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        """سازگاری با کد قبلی: فقط ماسک را برمی‌گرداند."""
        return self._build_text_mask_ex(image, regions)[0]

    def _build_text_mask_ex(self, image: np.ndarray, regions: List[TextRegion]
                            ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        """برمی‌گرداند: (mask, size_map). size_map ارتفاع متنِ مالکِ هر پیکسل ماسک (uint8، ۰=نامعلوم)
        یا None در حالت legacy."""
        if not self._mcfg().adaptive_mask:
            return self._build_text_mask_legacy(image, regions), None
        try:
            return self._build_text_mask_adaptive(image, regions)
        except Exception as e:
            print(f"  [!] ماسک تطبیقی ناموفق ({e}) → ماسک قدیمی.")
            return self._build_text_mask_legacy(image, regions), None

    def _build_text_mask_adaptive(self, image: np.ndarray, regions: List[TextRegion]
                                  ) -> Tuple[np.ndarray, np.ndarray]:
        cfg = self._mcfg()
        H, W = image.shape[:2]
        res_f = self._res_scale(H, W, 0.5, 0.85, 1.25)
        ...
        for region in regions:
            ocr_polys = list(getattr(region, "ocr_polys", None) or [])
            polys = ocr_polys or list(getattr(region, "boxes", None) or [])
            pts_list = []
            for poly in polys:
                try:
            ...
            if not pts_list:
                continue
            cv2.fillPoly(text_mask, pts_list, 255)
            allp = np.concatenate([p.reshape(-1, 2) for p in pts_list])
            x0, y0 = allp.min(axis=0)
            x1, y1 = allp.max(axis=0) + 1
            infos.append({
                "bbox": (int(x0), int(y0), int(x1), int(y1)),
                "pts": pts_list,
                # اندازه‌ی متن فقط وقتی معتبر است که از خطوط واقعیِ OCR آمده باشد، نه باکس کل bubble
                "th": self._poly_text_height(pts_list) if ocr_polys else 0.0,
                "promo": getattr(region, "kind", "") in ("promo", "sfx"),
            })
        if not infos or not np.any(text_mask):
            return text_mask, size_map

        # ۲) refine (بدون تغییر نسبت به قبل؛ اگر pydensecrf نباشد ماسک دست‌نخورده برمی‌گردد)
        base_mask = self._refine_text_mask_tight(image, text_mask)
        out_mask = base_mask.copy()

        # ۳) halo رنگی + ۴) padding تطبیقی، برای هر region روی ROI محلی
        big_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
        for inf in infos:
            th = float(inf["th"])
            pad = self._adaptive_pad_px(th, res_f)
            band = self._halo_band_px(th)
            margin = band + pad + 16 + (self._PROMO_R if inf["promo"] else 0)
            bx0, by0, bx1, by1 = inf["bbox"]
            rx0, ry0 = max(0, bx0 - margin), max(0, by0 - margin)
            rx1, ry1 = min(W, bx1 + margin), min(H, by1 + margin)
            if rx1 <= rx0 or ry1 <= ry0:
                continue

            m0 = base_mask[ry0:ry1, rx0:rx1]
            m1 = m0.copy()
            halo = self._halo_candidates(image[ry0:ry1, rx0:rx1], m0, band)
            if halo is not None:
                m1 = cv2.bitwise_or(m1, halo)

            if inf["promo"]:
                # رفتار قبلیِ promo/sfx (dilation بزرگ) حفظ شده است
                raw = np.zeros_like(m0)
                off = np.array([rx0, ry0], dtype=np.int32).reshape(1, 1, 2)
                cv2.fillPoly(raw, [p - off for p in inf["pts"]], 255)
                m1 = cv2.bitwise_or(m1, cv2.dilate(raw, big_kernel, iterations=3))

            base_pad = int(max(0, int(getattr(self, "mask_padding", 3) or 3)))
            if cfg.textured_keeps_base_pad and pad < base_pad and self._bg_is_textured(image[ry0:ry1, rx0:rx1], m1):
                pad = base_pad
            if pad > 0:
                k = 2 * pad + 1
                m2 = cv2.dilate(m1, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k)))
            else:
                m2 = m1

            out_mask[ry0:ry1, rx0:rx1] = cv2.bitwise_or(out_mask[ry0:ry1, rx0:rx1], m2)
            th_u8 = int(min(255, round(th)))
            if th_u8 > 0:
                sm = size_map[ry0:ry1, rx0:rx1]
                np.maximum(sm, np.where(m2 > 0, np.uint8(th_u8), np.uint8(0)), out=sm)
        return out_mask, size_map

    # ── flat-fill محلی (همان منطق قبلی، ولی روی crop و با حلقه‌ی تمیزتر) ──────────
    def _flat_fill_components_local(self, image: np.ndarray, mask: np.ndarray,
                                    exclude: Optional[np.ndarray] = None
                                    ) -> Tuple[np.ndarray, np.ndarray, int]:
        """
        مثل _flat_fill_uniform_components، با این تفاوت‌ها:
          • همه‌ی محاسبات محلی است (سریع، روی تصاویر بلند هم).
          • پیکسل‌های ماسکِ در انتظار (exclude) از حلقه‌ی نمونه‌برداری حذف می‌شوند.
          • (flat_robust) چند پیکسل دیوار/لبه در حلقه‌ی دور تحمل می‌شود، به شرط آنکه
            همسایگیِ ≤۳px ماسک کاملاً یک‌دست باشد (ماسک به دیوار نچسبیده باشد).
          • پیکسل‌های خودِ ماسک همیشه کامل رنگ می‌شوند؛ feather فقط به بیرون نرم می‌شود.
        خروجی: (filled, remaining_mask, تعداد component پرشده)
        """
        cfg = self._mcfg()
        filled = image.copy()
        ...
        near_k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7))
        excl = (exclude > 0) if exclude is not None else None
        n_ok = 0

        for i in range(1, n):
            x, y = int(stats[i, cv2.CC_STAT_LEFT]), int(stats[i, cv2.CC_STAT_TOP])
            w, h = int(stats[i, cv2.CC_STAT_WIDTH]), int(stats[i, cv2.CC_STAT_HEIGHT])
            wx0, wy0 = max(0, x - 11), max(0, y - 11)
            wx1, wy1 = min(W, x + w + 11), min(H, y + h + 11)
            comp = labels[wy0:wy1, wx0:wx1] == i
            comp_u8 = comp.astype(np.uint8) * 255
            win_img = image[wy0:wy1, wx0:wx1]
            win_excl = excl[wy0:wy1, wx0:wx1] if excl is not None else None

            def _reject():
                remaining[wy0:wy1, wx0:wx1][comp] = 255

            ring = (cv2.dilate(comp_u8, ring_k) > 0) & ~comp
            if win_excl is not None:
                ring &= ~win_excl
            if int(np.count_nonzero(ring)) < cfg.flat_min_ring_px:
                _reject()
                continue

            ring_px = win_img[ring].astype(np.float32)
            bg = None
            if float(ring_px.std(axis=0).max()) <= cfg.flat_std_thresh:
                bg = np.median(ring_px, axis=0)
            elif cfg.flat_robust:
                med = np.median(ring_px, axis=0)
                inl = np.abs(ring_px - med).max(axis=1) <= cfg.flat_inlier_tol
                if (float(inl.mean()) >= cfg.flat_inlier_frac
                        and float(ring_px[inl].std(axis=0).max()) <= cfg.flat_std_thresh):
                    near = (cv2.dilate(comp_u8, near_k) > 0) & ~comp
                    if win_excl is not None:
                        near &= ~win_excl
                    if int(np.count_nonzero(near)) >= 8:
                        npx = win_img[near].astype(np.float32)
                        if float((np.abs(npx - med).max(axis=1) <= cfg.flat_inlier_tol).mean()) >= 0.97:
                            bg = np.median(ring_px[inl], axis=0)
            if bg is None:
                _reject()
                continue

            pad = 4
            fx0, fy0 = max(0, x - pad), max(0, y - pad)
            fx1, fy1 = min(W, x + w + pad), min(H, y + h + pad)
            crop_comp = (labels[fy0:fy1, fx0:fx1] == i).astype(np.float32)
            feather = np.clip(cv2.GaussianBlur(crop_comp, (7, 7), 0), 0.0, 1.0)
            feather = np.maximum(feather, crop_comp)[..., None]
            crop_fill = np.full((fy1 - fy0, fx1 - fx0, 3), bg, dtype=np.float32)
            crop_out = filled[fy0:fy1, fx0:fx1].astype(np.float32)
            crop_out = crop_out * (1.0 - feather) + crop_fill * feather
            filled[fy0:fy1, fx0:fx1] = np.clip(crop_out, 0, 255).astype(np.uint8)
            n_ok += 1
        return filled, remaining, n_ok

    # ── cluster ها ───────────────────────────────────────────────────────
    def _adaptive_clusters(self, mask: np.ndarray, size_map: Optional[np.ndarray], res_lin: float):
        """
        ماسک را به cluster تقسیم می‌کند: هر پیکسل به‌اندازه‌ی «فاصله‌ی ادغامِ خودش»
        (نسبت × ارتفاع متنِ همان پیکسل) dilate می‌شود و connected components گرفته می‌شود.
        خروجی: (clusters, lab, (rx0, ry0)) — هر cluster: id, box=(x0,y0,x1,y1), th
        """
        cfg = self._mcfg()
        H, W = mask.shape
        m = mask > 0
        ...
        if ys.size == 0:
            return [], None, (0, 0)
        gmax = cfg.cluster_gap_max
        rx0, ry0 = max(0, int(xs.min()) - gmax), max(0, int(ys.min()) - gmax)
        rx1, ry1 = min(W, int(xs.max()) + 1 + gmax), min(H, int(ys.max()) + 1 + gmax)
        sub = m[ry0:ry1, rx0:rx1]
        if size_map is not None:
            th = size_map[ry0:ry1, rx0:rx1].astype(np.float32)
        else:
            th = np.zeros(sub.shape, dtype=np.float32)
        th = np.where(th > 0, th, self._NOMINAL_TEXT_H * res_lin)
        gap = np.clip(np.rint(cfg.cluster_gap_ratio * th), cfg.cluster_gap_min, cfg.cluster_gap_max)

        levels = sorted({int(min(cfg.cluster_gap_max, max(cfg.cluster_gap_min, v)))
                         for v in (6, 8, 12, 16, 24, 32, 40, cfg.cluster_gap_max)})
        lv = np.array(levels, dtype=np.float32)
        lvl_idx = np.minimum(np.searchsorted(lv, gap), len(lv) - 1)

        zone = np.zeros(sub.shape, dtype=np.uint8)
        for k, g in enumerate(levels):
            sel = sub & (lvl_idx == k)
            if not sel.any():
                continue
            yy, xx = np.nonzero(sel)
            y0, y1 = max(0, int(yy.min()) - g), min(sub.shape[0], int(yy.max()) + 1 + g)
            x0, x1 = max(0, int(xx.min()) - g), min(sub.shape[1], int(xx.max()) + 1 + g)
            d = cv2.dilate(sel[y0:y1, x0:x1].astype(np.uint8) * 255, np.ones((2 * g + 1, 2 * g + 1), np.uint8))
            zone[y0:y1, x0:x1] = cv2.bitwise_or(zone[y0:y1, x0:x1], d)

        n, lab, stats, _ = cv2.connectedComponentsWithStats(zone, connectivity=8)
        clusters = []
        for i in range(1, n):
            zx, zy = int(stats[i, cv2.CC_STAT_LEFT]), int(stats[i, cv2.CC_STAT_TOP])
            zw, zh = int(stats[i, cv2.CC_STAT_WIDTH]), int(stats[i, cv2.CC_STAT_HEIGHT])
            own = (lab[zy:zy + zh, zx:zx + zw] == i) & sub[zy:zy + zh, zx:zx + zw]
            oy, ox = np.nonzero(own)
            if oy.size == 0:
                continue
            x0, x1 = rx0 + zx + int(ox.min()), rx0 + zx + int(ox.max()) + 1
            y0, y1 = ry0 + zy + int(oy.min()), ry0 + zy + int(oy.max()) + 1
            cth = 0.0
            if size_map is not None:
                vals = size_map[ry0 + zy:ry0 + zy + zh, rx0 + zx:rx0 + zx + zw][own]
                vals = vals[vals > 0]
                if vals.size:
                    cth = float(np.median(vals))
            clusters.append({"id": i, "box": (x0, y0, x1, y1), "th": cth})
        clusters.sort(key=lambda c: (c["box"][1], c["box"][0]))
        return clusters, lab, (rx0, ry0)

    def _cluster_context_px(self, cw: int, ch: int, th: float, res_lin: float) -> int:
        """context محلی: متناسب با اندازه‌ی cluster، با کف/سقف وابسته به resolution و ارتفاع متن."""
        cfg = self._mcfg()
        lo = cfg.context_min * res_lin
        if th > 0:
            lo = max(lo, 2.0 * th)
        hi = max(lo, cfg.context_max * res_lin)
        return int(round(min(hi, max(lo, cfg.context_ratio * max(cw, ch)))))

    @staticmethod
    def _grow_to_min(a0: int, a1: int, limit: int, min_side: int) -> Tuple[int, int]:
        need = min_side - (a1 - a0)
        if need <= 0:
            return a0, a1
        a0 = max(0, a0 - need // 2)
        a1 = min(limit, a1 + (need - need // 2))
        if a1 - a0 < min_side:
            a0 = max(0, a1 - min_side)
            a1 = min(limit, a0 + min_side)
        return a0, a1

    def _clean_by_clusters(self, image: np.ndarray, mask: np.ndarray,
                           size_map: Optional[np.ndarray]) -> np.ndarray:
        """
        برای هر cluster: crop با context محلی → flat-fill (زمینه‌ی ساده) → LaMa → OpenCV.
        فقط پیکسل‌های ماسکِ همان cluster (و حلقه‌ی ۲pxیِ OpenCV) به تصویر برمی‌گردند؛
        بقیه‌ی تصویر بایت‌به‌بایت دست‌نخورده می‌ماند. ماسک clusterهای منتظرِ پردازش
        داخل hole قرار می‌گیرد تا متنِ آن‌ها به‌عنوان context به LaMa نرسد؛ clusterهای
        تمام‌شده هم به‌صورت تمیز در context دیده می‌شوند.
        """
        H, W = mask.shape
        res_lin = self._res_scale(H, W, 1.0, 0.6, 2.0)
        clusters, lab, (rx0, ry0) = self._adaptive_clusters(mask, size_map, res_lin)
        cleaned = image.copy()
        todo = ((mask > 0).astype(np.uint8)) * 255
        ...
        for cl in clusters:
            x0, y0, x1, y1 = cl["box"]
            ctx = self._cluster_context_px(x1 - x0, y1 - y0, cl["th"], res_lin)
            cx0, cx1 = self._grow_to_min(max(0, x0 - ctx), min(W, x1 + ctx), W, self._MIN_CROP_SIDE)
            cy0, cy1 = self._grow_to_min(max(0, y0 - ctx), min(H, y1 + ctx), H, self._MIN_CROP_SIDE)

            own_box = ((lab[y0 - ry0:y1 - ry0, x0 - rx0:x1 - rx0] == cl["id"])
                       & (mask[y0:y1, x0:x1] > 0))
            own = np.zeros((cy1 - cy0, cx1 - cx0), dtype=np.uint8)
            own[y0 - cy0:y1 - cy0, x0 - cx0:x1 - cx0][own_box] = 255

            pend = todo[cy0:cy1, cx0:cx1]                     # view روی todo
            crop_f, remaining, n_flat = self._flat_fill_components_local(
                cleaned[cy0:cy1, cx0:cx1], own, exclude=pend)
            counts["flat"] += n_flat
            if n_flat:
                cleaned[cy0:cy1, cx0:cx1] = crop_f
                pend[(own > 0) & (remaining == 0)] = 0

            if np.any(remaining):
                done = False
                if getattr(self, "use_lama", False) and not lama_loaded:
                    lama_loaded = True
                    lama = self._get_lama()
                if lama is not None:
                    try:
                        hole = pend.copy()                    # remaining + clusterهای منتظر
                        res_pil = lama(crop_f, hole)
                        out = cv2.cvtColor(np.array(res_pil), cv2.COLOR_RGB2BGR)
                        if out.shape[:2] != crop_f.shape[:2]:
                            out = cv2.resize(out, (crop_f.shape[1], crop_f.shape[0]))
                        mm = remaining > 0
                        view = cleaned[cy0:cy1, cx0:cx1]
                        view[mm] = out[mm]
                        counts["LaMa"] += 1
                        done = True
                    except Exception as e:
                        print(f"  [!] {getattr(self, '_inpainter_name', 'SimpleLama')} خطا ({e}) → OpenCV برای این cluster.")
                if not done:
                    cleaned[cy0:cy1, cx0:cx1] = self._opencv_inpaint_two_pass(
                        cleaned[cy0:cy1, cx0:cx1].copy(), remaining)
                    counts["OpenCV"] += 1

            todo[y0:y1, x0:x1][own_box] = 0

        print(f"  - Cluster cleanup: {len(clusters)} cluster | flat={counts['flat']} comp, "
              f"LaMa={counts['LaMa']}, OpenCV={counts['OpenCV']}")
        return cleaned

    def _build_text_mask_legacy(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        mask, size_map = self._build_text_mask_ex(image, regions)
        if not np.any(mask):
            return image.copy()
        ...
        if self._mcfg().cluster_clean:
            try:
                return self._clean_by_clusters(image, mask, size_map)
            except Exception as e:
                print(f"  [!] پاک‌سازی cluster-محور ناموفق ({e}) → پاک‌سازی سراسری قدیمی.")
        return self._clean_page_level(image, mask)

    def _clean_page_level(self, image: np.ndarray, mask: np.ndarray) -> np.ndarray:
...
- اگه بخشی از OCR خراب بود، فقط غلط‌های واضح حروف/املای همون کلمه رو اصلاح کن (مثلاً MUDIYING→MODIFYING)؛ جمله یا مفهوم تازه نساز و جمله‌ی ناقص رو کامل نکن.
...
اسم خاص رو فقط نویسه‌گردانی کن (تلفظ فارسیش رو بنویس)، ترجمه نکن. اگه املای اسم توی OCR خراب بود، فقط غلط واضح حروفش رو اصلاح کن؛ از رو بافت اسم یا معنی تازه‌ای نساز.
...
📝 قوانین وفاداری به متن OCR (اجباری):
- فقط متن موجود در OCR را ترجمه کن؛ هرگز از خودت جمله، کلمه یا مفهوم جدید اضافه نکن و از روی context یا تصویر حدس نزن.
- ترجمه باید فارسی طبیعی و روان باشد؛ ترتیب کلمات را برای درست شدن جمله‌بندی فارسی اصلاح کن، اما معنی جدیدی اضافه نکن و چیزی از متن اصلی حذف نکن.
- ترجمه تحت‌اللفظی و کلمه‌به‌کلمه نباشد، اما بازنویسی آزاد هم نباشد.
- اگر جمله ناقص است، ناقص بماند و کامل نشود.
- اگر متن فقط علائمی مثل "...", "......?!" یا "?!..." است، دقیقاً همان علائم را حفظ کن و هیچ کلمه‌ای اضافه نکن.
- لحن، زمان فعل، مکث‌ها و علائم مهم متن اصلی را حفظ کن.
- طبیعی‌سازی فارسی فقط برای درست شدن جمله‌بندی مجاز است، نه برای اضافه کردن اطلاعات یا ساختن دیالوگ جدید.

مثال جمله‌بندی:
اگر ترجمه تحت‌اللفظی یا بدچینش نتیجه‌ای مثل:
«انگار هوا توی آسمون معلق مونده...»
ایجاد کرد، جمله را به شکل طبیعی و درست فارسی بنویس:
«انگار ذرات هوا معلق موندن....»

مثال متن ناقص:
"Where are..."
→ «کجا...»

نه اینکه آن را کامل کنی و بنویسی:
«کجا رفتی؟»
...
و نباید هیچ جمله‌ای به آن اضافه شود.

مثال‌های بیشتر (ناقص، مکث‌دار، لکنت، غلط واضح حروف OCR):
"But if we..." → «ولی اگه ما...»
"I thought that..." → «فکر می‌کردم که...»
"It's not like I..." → «اینطور نیست که من...»
"W-wait...!" → «ص-صبر کن...!»
"Hmm..." → «هوم...»
"I DON'T KNOW WHAT YOU'RE TALKIING ABOUT" → «نمی‌دونم درباره‌ی چی حرف می‌زنی»
(در مثال آخر فقط غلط واضح حروف اصلاح شده؛ هیچ چیزی به معنی اضافه نشده.)

اصل اصلی:
«طبیعی ترجمه کن، ولی چیزی از خودت اضافه نکن.»

قانون اولویت:
اگر هر دستور قدیمی با قوانین جدید این بخش درباره حدس زدن یا تکمیل متن OCR تناقض داشت، قوانین جدید اولویت دارند. در نتیجه، برای متن OCR ناقص، خراب، مبهم یا ناخوانا، هرگز از context یا تصویر برای ساختن یا تکمیل جمله استفاده نکن. فقط همان اطلاعاتی را ترجمه کن که واقعاً در متن OCR وجود دارد.
این اولویت فقط برای جلوگیری از حدس زدن و ساختن محتوای جدید است و سایر قابلیت‌های prompt، مثل طبیعی بودن ترجمه، حفظ لحن، ترجمه (نویسه‌گردانی) اسم‌های خاص و جمله‌بندی صحیح فارسی را تغییر نمی‌دهد.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--no-adaptive-mask", action="store_true",
                   help="padding/halo ثابتِ قدیمی (بدون تطبیق با اندازه‌ی متن)")
    p.add_argument("--no-cluster-clean", action="store_true",
                   help="پاک‌سازی سراسریِ قدیمی به‌جای cluster-به‌cluster")
    p.add_argument("--mask-pad-min", type=int, default=None, help="کفِ padding تطبیقی (پیش‌فرض ۲)")
    p.add_argument("--mask-pad-max", type=int, default=None, help="سقفِ padding تطبیقی (پیش‌فرض ۶)")
    p.add_argument("--context-ratio", type=float, default=None,
                   help="context هر cluster = نسبت × بُعد بزرگ‌ترِ cluster (پیش‌فرض ۰.۵)")
    p.add_argument("--context-min", type=int, default=None, help="حداقل context به px (پیش‌فرض ۲۴)")
    p.add_argument("--context-max", type=int, default=None, help="حداکثر context به px (پیش‌فرض ۹۶)")
    p.add_argument("--mask-opt", action="append", default=[], metavar="KEY=VALUE",
                   help="تنظیم هر فیلدِ MaskCleanConfig، مثلاً --mask-opt halo_any_hue=1 "
                        "--mask-opt flat_std_thresh=16 (قابل تکرار)")
...
def main():
    ...
    mask_cfg = MaskCleanConfig(
        adaptive_mask=not getattr(args, "no_adaptive_mask", False),
        cluster_clean=not getattr(args, "no_cluster_clean", False),
    )
    for _attr, _field in (("mask_pad_min", "pad_min"), ("mask_pad_max", "pad_max"),
                          ("context_ratio", "context_ratio"), ("context_min", "context_min"),
                          ("context_max", "context_max")):
        _v = getattr(args, _attr, None)
        if _v is not None:
            setattr(mask_cfg, _field, _v)
    try:
        mask_cfg.apply_overrides(getattr(args, "mask_opt", None))
