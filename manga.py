...
class MangaTranslator:
    ...
    def __init__(
        ...
        self.bubbles_per_request = max(1, int(bubbles_per_request or 6))
        ...
        self.proofread_max_items_per_request = 25
...
ترجمه ثبت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def proofread_chapter(self, pages: List[Tuple[int, List["TextRegion"]]]) -> None:
        ...
        max_batch = max(1, int(getattr(self, "proofread_max_items_per_request", 25) or 25))
        ...
        if len(batches) > 1:
            print(f"    [*] بازبینی یکپارچهٔ فصل: {len(pages)} صفحه / {len(all_items)} دیالوگ "
        ...
        def _process_group(items: List[Tuple[int, int, str]], depth: int = 0) -> None:
            ...
            for (p, i, _t) in items:
                ...
                if corrected and corrected != (region.translated_text or "").strip():
                    ...
                    applied += 1
            if applied:
                print(f"    [+] بازبینی: {applied}/{len(items)} دیالوگ اصلاح شد.")
            else:
                print(f"    [+] بازبینی: {len(items)} دیالوگ بررسی شد — نیازی به تغییر نبود.")
