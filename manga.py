...
class MangaTranslator:
    ...
    def __init__(
        ...
        self.proofread_enabled = bool(proofread)
        self.proofread_max_split_depth = 6
        self.proofread_max_items_per_request = 40
        
        self.translate_safe_context_tokens = 16000
        ...
        if not text_only and (not font_path or not os.path.isfile(font_path)):
        ...
        if text_only:
            ...
            if gpu is None:
                self.use_gpu = self._detect_paddle_gpu() or self._detect_torch_cuda() or _ort_has_cuda()
            else:
            ...
            self.det = None
            print("[*] حالت text_only: OCR/تشخیص حباب لود نمی‌شن (رندر/inpaint در صورت نیاز عادی کار می‌کنه).")
        else:
            if gpu is None:
    ...
    def _get_system_instruction(self, include_page: bool = False) -> str:
        if include_page:
            fmt_block = """━━━━━━━━━━━━━━━━━━━━
فرمت خروجی (الزامی)
━━━━━━━━━━━━━━━━━━━━
هیچ توضیح، تحلیل یا متن اضافه ننویس؛ فقط JSON معتبر برگردون.
ورودی یه آرایه‌ست شامل حباب‌های چند صفحه‌ی یه فصل کامل، با فیلدهای page, id, text.
خروجی هم باید یه آرایه از همون آیتم‌ها باشه، هر آیتم دقیقاً به این شکل:
{"page": عدد (همون page ورودی), "id": عدد (همون id ورودی), "translation": "متن فارسی",
"tone": "لحن", "names": [{"source": "...", "persian": "..."}]}
page و id باید عیناً همون مقدار ورودی برگردن تا بشه هر ترجمه رو به حباب اصلیش وصل کرد."""
        else:
            fmt_block = """━━━━━━━━━━━━━━━━━━━━
فرمت خروجی (الزامی)
━━━━━━━━━━━━━━━━━━━━
...
🔄 بازچینی ترتیب کلمات (اجباری، جدا از لحن):
- اول کل جمله رو بفهم، بعد از صفر با چینش خود فارسی بنویسش؛ چینش کلمات زبان مبدأ رو روی فارسی کپی نکن.
- فعل معمولاً آخر جمله بیاد، قید/متمم/مفعول قبل از فعل بشینه؛ اگه ترتیب زبان مبدأ با این فرق داره، حتماً جابه‌جا کن.
- غلط (چینش کلمه‌به‌کلمه): «با من بیا به آزمایشگاهم» | درست: «با من به آزمایشگاهم بیا» یا محاوره‌ای‌تر: «بیا بریم آزمایشگاهم»
- غلط: «من فکر می‌کنم که تو درست می‌گی» | درست: «فکر می‌کنم راست می‌گی»
- غلط: «نگو به من که باید چیکار کنم» | درست: «نگو من چیکار کنم»
- تست سریع قبل از تحویل هر جمله: اگه یه فارسی‌زبان می‌خواست همین حرف رو مستقیم به فارسی بزنه (نه از رو یه متن دیگه)، دقیقاً همین چینش رو به‌کار می‌برد؟ اگه جواب نه‌ست، جمله رو بازچینی کن. معنی، اطلاعات و لحن باید کاملاً همون بمونه؛ فقط چینش/ساختار عوض می‌شه.
...
ترجمه ثبت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def _parse_translation_response(self, text: str, regions: List[TextRegion]) -> bool:
        ...
        applied = 0
        valid_tones = self._VALID_TONES
        tone_aliases = self._TONE_ALIASES
        for region in regions:
    ...
    def _chapter_translate_with_gemini(self, user_prompt: str, system_instruction: str) -> str:
        ...
        config = genai_types.GenerateContentConfig(
            ...
            response_schema={
                ...
                "items": {
                    ...
                    "required": ["page", "id", "translation", "tone"],
                },
            },
        )
    ...
    def _chapter_translate_with_openai(self, user_prompt: str, system_instruction: str) -> str:
        ...
        kwargs = dict(
            ...
            temperature=self.translation_temperature,
            timeout=float(getattr(self, "api_timeout", 30.0) or 30.0),
        )
        mlow = self.model_name.lower()
        if any(x in mlow for x in ("gpt-4", "gpt-3.5", "gpt-5", "o1", "o3", "o4")):
            kwargs["response_format"] = {"type": "json_object"}
    ...
    def _proofread_system_instruction(self) -> str:
        return (
            "تو یه ویراستار حرفه‌ای ترجمه‌ی مانگا/مانهوا هستی. کل فصل قبلاً به فارسی "
            "ترجمه شده؛ کارت اینه که به‌عنوان آخرین مرحله، کل ترجمه‌ی فصل رو یکپارچه و "
            "یک‌جا (نه صفحه‌به‌صفحه و نه دیالوگ‌به‌دیالوگ) بازبینی و در صورت نیاز اصلاح "
            "کنی — دقیقاً مثل مرحله‌ی ویراستاری نهایی قبل از انتشار که یه ویراستار "
            "انسانی روی کل فصل انجام می‌ده.\n"
            "این موارد رو توی کل فصل چک و در صورت نیاز اصلاح کن:\n"
            "1. غلط املایی\n"
            "2. غلط تایپی\n"
            "3. نیم‌فاصله (مثل «می‌خوام» نه «میخوام» یا «می خوام»)\n"
            "4. فاصله‌گذاری استاندارد فارسی (کلمات به‌هم‌چسبیده یا فاصله‌ی اضافی)\n"
            "5. علائم نگارشی استاندارد (، ! ؟ .)\n"
            "6. طبیعی و محاوره‌ای بودن جمله‌ها (نه کتابی/رسمی)\n"
            "7. یکدست بودن لحن هر شخصیت در طول کل فصل\n"
            "8. یکدست بودن اسم‌ها در همه‌ی صفحه‌ها\n"
            "9. یکدست بودن اصطلاح‌های خاص داستان در همه‌ی صفحه‌ها\n"
            "10. تناقض بین صفحه‌های مختلف (اسمی/لحنی که ناخواسته بین صفحه‌ها عوض شده)\n"
            "11. ترجمه‌ی تحت‌اللفظی یا ماشینی که باید به فارسیِ طبیعی بازنویسی بشه\n"
            "12. حفظ کامل معنی اصلی؛ چیزی به معنی اضافه یا کم نکن\n"
            + self._glossary_prompt_block()
            + self._brief_prompt_block()
            + "\n"
            "واژه‌نامه و خلاصه‌ی داستان بالا (در صورت وجود) نسبت به هر ابهامی اولویت "
            "مطلق دارن؛ اسم‌ها و اصطلاح‌های داخل واژه‌نامه رو عیناً همون‌طور نگه دار و "
            "فقط با توجه بهشون یکدستی رو بین صفحه‌ها تضمین کن.\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "فرمت خروجی (الزامی)\n"
            "━━━━━━━━━━━━━━━━━━━━\n"
            "ورودی یه آرایه‌ست شامل دیالوگ‌های ترجمه‌شده‌ی کل فصل با فیلدهای page, id, text.\n"
            "برای تک‌تک آیتم‌های ورودی، دقیقاً یک آیتم خروجی بده:\n"
            "{\"page\": همون page ورودی, \"id\": همون id ورودی, "
            "\"has_error\": true/false, \"corrected\": \"متن نهایی فارسی\"}\n"
            "اگه دیالوگ از قبل درست، طبیعی و بدون غلط بود، has_error را false بذار و "
            "corrected را دقیقاً عین متن ورودی (بدون کوچک‌ترین تغییر) برگردان؛ بدون "
            "دلیل واقعی هیچ دیالوگی رو بازنویسی نکن. فقط وقتی has_error=true است که "
            "corrected باید متفاوت از متن ورودی باشد.\n"
            "هیچ توضیح، تحلیل یا متن اضافه ننویس؛ فقط همون آرایه‌ی JSON را برگردان."
        )

    @staticmethod
    def _proofread_response_schema() -> dict:
        return {
            "type": "ARRAY",
            "items": {
                "type": "OBJECT",
                ...
                "required": ["page", "id", "has_error", "corrected"],
            },
        }

    def _proofread_with_gemini(self, user_prompt: str, system_instruction: str) -> str:
        config = genai_types.GenerateContentConfig(
            system_instruction=system_instruction,
            response_mime_type="application/json",
            temperature=min(float(getattr(self, "translation_temperature", 0.85) or 0.85), 0.4),
            response_schema=self._proofread_response_schema(),
        )
        ...
        return self._call_ai_with_timeout(_do, label=f"Proofread/{self._thread_model()}")

    def _proofread_with_openai(self, user_prompt: str, system_instruction: str) -> str:
        kwargs = dict(
            model=self.model_name,
            messages=[
                {"role": "system", "content": system_instruction},
        ...
        return self._call_ai_with_timeout(_do, label=f"Proofread/{self.provider}")
    ...
    def _is_context_too_large_error(err: Exception) -> bool:
        ...
        markers = (
            ...
            "maximum context", "too many tokens", "token count",
            "exceeds the maximum", "input is too long", "content too long",
            "payload too large", "request entity too large",
            "maximum number of tokens", "exceeds maximum",
            "too large for the model", "prompt is too long",
            "resource has been exhausted (e.g. check quota)",
        )
        if any(m in low for m in markers):
            return True
        if ("400" in msg or "invalid_argument" in low) and (
            "token" in low or "context" in low or "too long" in low
        ):
            return True
        if "413" in msg:
            return True
    ...
    @staticmethod
    def _is_deadline_exceeded_error(err: Exception) -> bool:
        
        
        
        msg = str(err).lower()
        markers = ("504", "deadline_exceeded", "deadline expired")
        return any(m in msg for m in markers)

    def _generate_ai_json_array(self, system_instruction: str, user_prompt: str,
                                 gemini_fn, openai_fn, label: str = "درخواست"):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                with self._api_lock:
                    if self.provider_type == "gemini":
                        text = gemini_fn(user_prompt, system_instruction)
                    else:
                        text = openai_fn(user_prompt, system_instruction)
                cleaned = text.strip()
                ...
                if not isinstance(result, list):
                    raise ValueError(f"پاسخ {label} آرایه نیست.")
                ...
                if self.provider_type == "gemini" and self._is_daily_quota_error(e):
                    raise GeminiQuotaExhausted(
                        f"سهمیهٔ روزانهٔ {self.model_name} تمام شد؛ {label} متوقف شد."
                    ) from e

                
                
                
                if self._is_context_too_large_error(e):
                    print(f"    [!] این Request بزرگ‌تر از Context Window مدل «{self.model_name}» "
                          f"است ({label}؛ نه خطای موقت) → تلاش مجدد بی‌فایده‌ست، این بخش نصف می‌شود.")
                    return None

                if self._is_deadline_exceeded_error(e):
                    deadline_streak += 1
                    if deadline_streak >= 2:
                        print(f"    [!] {label}: سرور دو بار پشت‌سرهم با 504 DEADLINE_EXCEEDED "
                              f"جواب داد (احتمالاً حجم درخواست زیاده) → به‌جای اتلاف "
                              f"{self.max_retries} تلاش، همین الان این بخش نصف می‌شود.")
                        return None
                    print(f"    [!] {label}: تلاش {attempt}/{self.max_retries} با 504 "
                          f"DEADLINE_EXCEEDED ناموفق شد — یک بار دیگر امتحان می‌شود.")
                ...
                if self._is_model_permanently_gone(e):
                    if self._drop_current_model_and_switch(reason=f"{label} model gone"):
                        self._recreate_api_client()
                        time.sleep(0.3)
                        continue
                if self._is_model_unavailable_error(e):
                    if self._switch_to_next_model(reason=f"{label} unavailable"):
                        time.sleep(0.2)
                        continue
                    if self._switch_to_next_key(reason=f"{label} model unavailable", cycle=True):
                        self._recreate_api_client()
                        time.sleep(0.3)
                        continue
                ...
                if self._is_rate_or_model_quota_error(e) or any(
                    ...
                    if self._switch_to_next_model(reason=f"{label} rate/quota"):
                        time.sleep(0.5)
                        continue
                    if self._switch_to_next_key(reason=f"{label} rate/quota", cycle=True):
                        self._recreate_api_client()
                        time.sleep(1.0)
                        continue
                ...
                is_timeout = (
                    isinstance(e, TimeoutError) or isinstance(e, FuturesTimeout)
                ...
                print(f"    [!] {label}: تلاش {attempt}/{self.max_retries} ناموفق (خطای موقت) — "
                      f"همین یک Request دوباره امتحان می‌شه: {last_err}")
                if attempt < self.max_retries:
                    time.sleep(delay)
                    delay = min(delay * 1.6, 8.0)

        print(f"    [!] {label} بعد از {self.max_retries} تلاش روی همین یک Request هم ناموفق موند "
              f"→ به‌عنوان آخرین راه‌حل این بخش نصف می‌شود.")
        return None

    def _generate_proofread_json(self, system_instruction: str, user_prompt: str):
        return self._generate_ai_json_array(
            system_instruction, user_prompt,
            self._proofread_with_gemini, self._proofread_with_openai,
            label="بازبینی",
        )

    def _generate_chapter_translate_json(self, system_instruction: str, user_prompt: str):
        return self._generate_ai_json_array(
            system_instruction, user_prompt,
            self._chapter_translate_with_gemini, self._chapter_translate_with_openai,
            label="ترجمهٔ فصل",
        )

    def _run_proofread_batches(
        self, all_items: List[Tuple[int, int, str]], label: str = "بازبینی یکپارچهٔ فصل",
        ...
        max_batch = max(1, int(getattr(self, "proofread_max_items_per_request", 40) or 40))
        n_batches = max(1, math.ceil(len(all_items) / max_batch))
        batches = [all_items[i::n_batches] for i in range(n_batches)] if n_batches > 1 else [all_items]
        batches = [b for b in batches if b]

        if len(batches) > 1:
            print(f"    [*] {label}: {len(all_items)} دیالوگ "
                  f"→ {len(batches)} بسته (حداکثر ~{max_batch} دیالوگ در هر Request تا به 504 "
                  f"DEADLINE_EXCEEDED نخوریم؛ تقسیم بیشتر فقط در صورت شکست)")
        else:
            print(f"    [*] {label}: {len(all_items)} دیالوگ "
                  f"(۱ درخواست؛ تقسیم فقط در صورت شکست)")

        system_instruction = self._proofread_system_instruction()

        def _process_group(items: List[Tuple[int, int, str]], depth: int = 0) -> None:
            if not items:
                return
            payload = [{"page": p, "id": i, "text": t} for (p, i, t) in items]
            user_prompt = (
                "این‌ها همهٔ دیالوگ‌های ترجمه‌شدهٔ یک فصل مانهوا هستند (از چند صفحه، به "
                "ترتیب page/id). به‌عنوان آخرین مرحله، کل این آرایه را یک‌جا و با دید کلی "
                "به کل فصل بازبینی و در صورت نیاز اصلاح کن.\n\n"
                f"{json.dumps(payload, ensure_ascii=False, indent=2)}\n\n"
                "فقط یک آرایهٔ JSON با فیلدهای page, id, has_error, corrected برای "
                "همهٔ آیتم‌های بالا برگردان."
            )
            result = self._generate_proofread_json(system_instruction, user_prompt)
            if result is None:
                if len(items) > 1 and depth < self.proofread_max_split_depth:
                    mid = len(items) // 2
                    print(f"    [!] بازبینی این بخش ({len(items)} دیالوگ) توی یک Request جا "
                          f"نشد/ناموفق بود → تقسیم به دو نیمه و ادامه")
                    _process_group(items[:mid], depth + 1)
                    _process_group(items[mid:], depth + 1)
                else:
                    print(f"    [!] بازبینی {len(items)} دیالوگ نهایی ناموفق ماند؛ "
                          f"همون‌طوری که بودن می‌مونن.")
                return
            ...
            for item in result:
                if not isinstance(item, dict):
                    continue
                try:
                    key = (int(item.get("page")), int(item.get("id")))
                except (TypeError, ValueError):
                    continue
                results[key] = item

        for batch in batches:
            _process_group(batch)

        return results

    def proofread_chapter(self, pages: List[Tuple[int, List["TextRegion"]]]) -> None:
        ...
        if not self.proofread_enabled or not pages:
            return

        lookup: Dict[Tuple[int, int], TextRegion] = {}
        for page_no, regions in pages:
            for r in regions:
                if (r.translated_text or "").strip():
                    lookup[(page_no, r.id)] = r

        if not lookup:
            return

        all_items = sorted(
            ((p, i, r.translated_text) for (p, i), r in lookup.items()),
            key=lambda x: (x[0], x[1]),
        )

        by_key = self._run_proofread_batches(all_items, label="بازبینی یکپارچهٔ فصل")

        applied = 0
        for (p, i, _t) in all_items:
            key = (p, i)
            region = lookup.get(key)
            item = by_key.get(key)
            if not region or not item:
                continue
            if not item.get("has_error"):
                continue
            corrected = str(item.get("corrected") or "").strip()
            if corrected and corrected != (region.translated_text or "").strip():
                region.translated_text = self._cleanup_translation(corrected)
                applied += 1
        if applied:
            print(f"    [+] بازبینی: {applied}/{len(all_items)} دیالوگ اصلاح شد.")
        else:
            print(f"    [+] بازبینی: {len(all_items)} دیالوگ بررسی شد — نیازی به تغییر نبود.")

    def correct_translations_from_json(self, json_path: str,
        ...
        if not isinstance(blocks, list) or not blocks:
            raise ValueError(
                "فایل JSON ورودی خالی یا فرمتش نامعتبره (باید آرایه‌ای از "
                "بلوک‌ها با page/id/original/translation باشه)."
            )
        ...
        for b in blocks:
            try:
                key = (int(b.get("page")), int(b.get("id")))
            except (TypeError, ValueError):
                continue
            t = (b.get("translation") or "").strip()
            if not t:
                continue
            all_items.append((key[0], key[1], t))
            lookup[key] = b

        by_key = self._run_proofread_batches(
            sorted(all_items, key=lambda x: (x[0], x[1])),
            label="بازبینی/اصلاح از روی JSON",
        )

        fixed = 0
        for key, b in lookup.items():
            item = by_key.get(key)
            if not item or not item.get("has_error"):
                continue
            corrected = str(item.get("corrected") or "").strip()
            if corrected and corrected != (b.get("translation") or "").strip():
                b["translation"] = self._cleanup_translation(corrected)
                fixed += 1

        out_path = out_path or (os.path.splitext(json_path)[0] + "_corrected.json")
        os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(blocks, f, ensure_ascii=False, indent=2)
        print(f"[✓] JSON اصلاح‌شده ذخیره شد در: {out_path} ({fixed} بلوک اصلاح شد).")
        return out_path
    ...
    def _estimate_tokens(cls, text: str) -> int:
        ...
        if not text:
            return 0
        return max(1, int(len(text) / cls._CHARS_PER_TOKEN_ESTIMATE) + 1)

    def _plan_chapter_translation_batches(
        ...
        system_instruction = self._get_system_instruction(include_page=True) + self._brief_prompt_block()
        shared_tokens = self._estimate_tokens(system_instruction)

        
        
        safe_budget = int(getattr(self, "translate_safe_context_tokens", 16000) or 16000)
        per_batch_budget = max(1500, safe_budget - shared_tokens)

        item_costs = []
        for (_p, _i, t) in items:
            src_tok = self._estimate_tokens(t or "")
            
            
            cost = src_tok + int(src_tok * 1.3) + 24
            item_costs.append(max(cost, 20))

        batches: List[List[Tuple[int, int, str]]] = []
        cur: List[Tuple[int, int, str]] = []
        cur_cost = 0
        for item, cost in zip(items, item_costs):
            if cur and (cur_cost + cost > per_batch_budget):
                batches.append(cur)
                cur = []
                cur_cost = 0
            cur.append(item)
    ...
    def _apply_chapter_translation_results(
        self, result: list, lookup: Dict[Tuple[int, int], TextRegion],
    ) -> int:
        ...
        for item in result:
            ...
            if t:
                region.translated_text = self._cleanup_translation(t)
                applied += 1
            st = (item.get("tone") or item.get("style") or item.get("bubble_style") or "")
            st = str(st).strip().lower().replace("-", "_").replace(" ", "_")
            st = tone_aliases.get(st, st)
            if st in valid_tones:
                region.bubble_style = st
    ...
    def translate_chapter(self, pages: List[Tuple[int, List[TextRegion]]]) -> None:
        ...
        for page_no, regions in pages:
            for r in regions:
                ...
                if not src:
                    continue
                r.source_text = self._fix_ocr_text(uncensor_swears(src))
                lookup[(page_no, r.id)] = r
                all_items.append((page_no, r.id, r.source_text))
                if len(self._brief_corpus) < 400:
        ...
        self._pick_random_api_key(reason="ترجمه فصل")
        ...
        batches = self._plan_chapter_translation_batches(all_items)
        print(f"    [*] ترجمهٔ یکپارچهٔ فصل: {len(pages)} صفحه / {len(all_items)} دیالوگ "
              f"→ {len(batches)} Batch بزرگ (token-aware؛ نه per-page)")

        system_instruction = self._get_system_instruction(include_page=True) + self._brief_prompt_block()

        def _process_batch(items: List[Tuple[int, int, str]], depth: int = 0) -> None:
            if not items:
                return
            payload = [{"page": p, "id": i, "text": t} for (p, i, t) in items]
            user_prompt = (
                "این‌ها همهٔ دیالوگ‌های خام (OCR) یک فصل مانهوا هستند (از چند صفحه، به "
                "ترتیب page/id). متن‌ها ممکن است غلط OCR داشته باشند؛ قبل از ترجمه، خودت "
                "در ذهنت تصحیحشون کن. با توجه به پیوستگی کل فصل (نه صفحه‌به‌صفحه)، هر "
                "مورد را به یک دیالوگ کاملاً طبیعی و محاوره‌ای فارسی بازآفرینی کن.\n\n"
                f"{json.dumps(payload, ensure_ascii=False, indent=2)}\n\n"
                "برای تک‌تک آیتم‌های ورودی، دقیقاً یک آیتم خروجی با فیلدهای "
                "page, id, translation, tone (و در صورت نیاز names) برگردان؛ page و id "
                "باید عیناً همون ورودی باشند."
            )
            result = self._generate_chapter_translate_json(system_instruction, user_prompt)
            if result is None:
                if len(items) > 1 and depth < self.proofread_max_split_depth:
                    mid = len(items) // 2
                    print(f"    [!] این Batch ترجمه ({len(items)} دیالوگ) توی یک Request جا "
                          f"نشد/ناموفق بود → تقسیم به دو نیمه و ادامه (بدون Request اضافه برای "
                          f"تشخیص خطا؛ فقط تقسیم برای جاشدن در Context)")
                    _process_batch(items[:mid], depth + 1)
                    _process_batch(items[mid:], depth + 1)
                else:
                    print(f"    [!] ترجمهٔ {len(items)} دیالوگ نهایی ناموفق ماند؛ "
                          f"بدون ترجمه می‌مونن (بدون Request اضافه).")
                return

            applied = self._apply_chapter_translation_results(result, lookup)
            print(f"    [+] {applied}/{len(items)} دیالوگ این Batch ترجمه شد.")

        for batch in batches:
            _process_batch(batch)

        still = sum(
            1 for (p, i, _t) in all_items
            if not (lookup[(p, i)].translated_text or "").strip()
        )
        if still:
            print(f"    [!] در نهایت {still} دیالوگ بدون ترجمه ماند "
                  f"(طبق طراحی، برای این‌ها Request اضافه‌ای زده نمی‌شه).")
    ...
    def translate_regions(self, regions: List[TextRegion]) -> None:
        ...
        def _make_batches(items: List[TextRegion]):
            ...
            max_items = max(1, int(getattr(self, "bubbles_per_request", 12) or 12))
            max_chars = 3200
    ...
    def translate_page_phase(self, regions: List[TextRegion]) -> List[TextRegion]:
    ...
    def _region_to_dict(r: "TextRegion") -> dict:
        ...
        return {
            "id": r.id,
            "boxes": [_arr(b) for b in (r.boxes or [])],
            "source_text": r.source_text,
            "translated_text": r.translated_text,
            "rect": list(r.rect),
            "angle": r.angle,
            "kind": r.kind,
            "bubble_style": r.bubble_style,
            "det_class": r.det_class,
            "ocr_polys": [_arr(p) for p in (r.ocr_polys or [])],
        }
    ...
    def _region_from_dict(d: dict) -> "TextRegion":
        ...
        return TextRegion(
            id=int(d.get("id", 0)),
            boxes=[np.array(b, dtype=np.int32) for b in (d.get("boxes") or [])],
            source_text=d.get("source_text", "") or "",
            translated_text=d.get("translated_text", "") or "",
            rect=tuple(d.get("rect") or (0, 0, 0, 0)),
            angle=float(d.get("angle", 0.0) or 0.0),
            kind=d.get("kind", "dialogue") or "dialogue",
            bubble_style=d.get("bubble_style", "normal") or "normal",
            det_class=d.get("det_class", "") or "",
            ocr_polys=[np.array(p, dtype=np.int32) for p in (d.get("ocr_polys") or [])],
        )

    def render_page_phase(self, image: np.ndarray, regions: List[TextRegion],
                           dbg: Optional[np.ndarray] = None,
                           ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
        
        if not regions:
            return image.copy(), None

        page_debug: Optional[np.ndarray] = dbg
        ...
        print("--- پاسخ نهایی برای هر بالن (بعد از ترجمه + بازبینی) ---")
        ...
        print("[فاز ۵ - پاکسازی متن + رندر نهایی] ...")
    ...
    def finish_page_phase(self, image: np.ndarray, regions: List[TextRegion],
        ...
        self.translate_page_phase(regions)

        return self.render_page_phase(image, regions)
...
"""
    ...
    def run(self, input_path: str, output_path: str, resume: bool = True,
        ...
        def _extract_one(item):
            ...
            try:
                ...
                if self._is_mostly_blank(image):
                    ...
                    return page_i, out_file, f, None, None, None
                print(f"[فاز ۱ - تشخیص حباب + OCR] '{basename}'...")
                regions, dbg = self.extract_regions_phase(image)
                return page_i, out_file, f, image, regions, dbg
            except GeminiQuotaExhausted:
                raise
            except Exception as e:
        ...
        def _render_one(page_i, out_file, f, regions, dbg):
            ...
            try:
                image = cv2.imread(f)
                if image is None:
                    print(f"    [!] بازخوانی تصویر برای رندر نهایی ناموفق: {f}", file=sys.stderr)
                    return page_i, out_file, None, dbg
                result, page_debug = self.render_page_phase(image, regions, dbg)
                dbg_out = page_debug if page_debug is not None else dbg
                return page_i, out_file, result, dbg_out
            except GeminiQuotaExhausted:
                raise
            except Exception as e:
                print(f"    [!] خطا در رندر {os.path.basename(out_file)}: {e}", file=sys.stderr)
                return page_i, out_file, None, dbg

        
        
        extracted_by_i: Dict[int, tuple] = {}

        if len(pending) <= 1:
            for item in pending:
                try:
                    page_i, out_file, f, image, regions, dbg = _extract_one(item)
                    del image
                except GeminiQuotaExhausted as e:
                    print(f"\n[!] {e}")
                    break
                extracted_by_i[page_i] = (out_file, f, regions, dbg)
        else:
            from concurrent.futures import FIRST_COMPLETED, wait as fut_wait
            from collections import deque
            print(
                f"[*] استخراج دوطرفه: اول از بالا→پایین + آخر از پایین→بالا "
                f"({len(pending)} صفحه؛ ترجمه بعداً یکجا برای کل فصل انجام می‌شه)"
            )
            extract_w = 2  
            with ThreadPoolExecutor(max_workers=extract_w) as extract_ex:
                ...
                _submit_extract()
                while extract_futs or q:
                    wait_set = set(extract_futs)
                    if not wait_set:
                    ...
                    for fut in done:
                        ...
                        try:
                            page_i, out_file, f, image, regions, dbg = fut.result()
                        except GeminiQuotaExhausted as e:
                            print(f"\n[!] {e}")
                            api_dead[0] = True
                            continue
        ...
        translate_pages: List[Tuple[int, List[TextRegion]]] = []
        for page_i in sorted(extracted_by_i.keys()):
            _, _, regions, _ = extracted_by_i[page_i]
            if not regions:
                continue
            dialogue_regions = [r for r in regions if r.kind == "dialogue"]
            if dialogue_regions:
                translate_pages.append((page_i + 1, dialogue_regions))

        if translate_pages:
            n_dialog = sum(len(rs) for _, rs in translate_pages)
            print(f"\n===== فاز ۳ — ترجمهٔ یکپارچهٔ کل فصل "
                  f"({len(translate_pages)} صفحه / {n_dialog} دیالوگ، مستقل از تعداد صفحه "
                  f"به چند Batch بزرگ token-aware تقسیم می‌شه) =====")
            try:
                self.translate_chapter(translate_pages)
            except GeminiQuotaExhausted as e:
                print(f"\n[!] {e}")
                print("    [!] ترجمهٔ فصل قبل از اتمام سهمیه متوقف شد؛ صفحات باقی‌مانده "
                      "بدون ترجمه رندر می‌شن.")
            except Exception as e:
                print(f"    [!] خطا در ترجمهٔ یکپارچهٔ فصل: {e}", file=sys.stderr)
        else:
            print("[فاز ۳ - ترجمه] هیچ دیالوگی در کل فصل پیدا نشد.")

        
        translated_by_i: Dict[int, tuple] = extracted_by_i
        ...
        render_items = [
            (page_i,) + translated_by_i[page_i] for page_i in sorted(translated_by_i.keys())
        ]

        proofread_pages: List[Tuple[int, List[TextRegion]]] = []
        for page_i, out_file, f, regions, dbg in render_items:
            if not regions:
                continue
            dialogue_regions = [
                r for r in regions
                if r.kind == "dialogue" and (r.translated_text or "").strip()
            ]
            if dialogue_regions:
                proofread_pages.append((page_i + 1, dialogue_regions))

        if self.proofread_enabled and proofread_pages:
            n_dialog = sum(len(rs) for _, rs in proofread_pages)
            print(f"\n===== فاز ۴ — بازبینی/ویرایش یکپارچهٔ کل فصل "
                  f"({len(proofread_pages)} صفحه / {n_dialog} دیالوگ) =====")
            try:
                self.proofread_chapter(proofread_pages)
            except GeminiQuotaExhausted as e:
                print(f"\n[!] {e}")
                print("    [!] بازبینی کلی فصل قبل از اتمام سهمیه متوقف شد؛ "
                      "با آخرین ترجمه‌های موجود ادامه می‌دیم.")
            except Exception as e:
                print(f"    [!] خطا در بازبینی کلی فصل: {e}", file=sys.stderr)
        elif self.proofread_enabled:
            print("[فاز ۴ - بازبینی] دیالوگ ترجمه‌شده‌ای برای بازبینی نبود.")
        else:
            print("[فاز ۴ - بازبینی] غیرفعال است (--no-proofread).")

        
        
        
        regions_cache_dir = os.path.join(cache_dir, "regions")
        os.makedirs(regions_cache_dir, exist_ok=True)
        for page_i, out_file, f, regions, dbg in render_items:
            if not regions:
                continue
            page_json = {
            ...
            cache_path = os.path.join(regions_cache_dir, f"page_{page_i + 1:04d}.json")
            try:
                with open(cache_path, "w", encoding="utf-8") as fh:
                    json.dump(page_json, fh, ensure_ascii=False)
            except Exception as e:
                print(f"    [!] ذخیرهٔ کش ناحیه‌های صفحهٔ {page_i + 1} ناموفق: {e}")

        if proofread_pages:
            translations_json_path = os.path.splitext(output_path)[0] + "_translations.json"
            data = [
                {"page": p, "id": r.id, "original": r.source_text, "translation": r.translated_text}
                for p, rs in proofread_pages for r in rs
            ]
            try:
                with open(translations_json_path, "w", encoding="utf-8") as fh:
                    json.dump(data, fh, ensure_ascii=False, indent=2)
                print(f"[✓] JSON کامل ترجمه‌ی فصل ذخیره شد در: {translations_json_path} "
                      f"({len(data)} دیالوگ) — برای اصلاح بعدی بدون تکرار OCR/رندر "
                      f"می‌تونی با --correct-json روی همین فایل کار کنی.")
            except Exception as e:
                print(f"    [!] ذخیره‌ی JSON ترجمه‌ی فصل ناموفق: {e}")

        if len(render_items) <= 1:
            for page_i, out_file, f, regions, dbg in render_items:
                try:
                    page_i, out_file, result, dbg = _render_one(page_i, out_file, f, regions, dbg)
                except GeminiQuotaExhausted as e:
                    print(f"\n[!] {e}")
                    break
                results_by_i[page_i] = (out_file, result, dbg)
        elif render_items:
            render_w = max(1, min(3, int(getattr(self, "batch_workers", 3) or 3)))
            print(f"[*] رندر نهایی موازی ({len(render_items)} صفحه، {render_w} worker)...")
            with ThreadPoolExecutor(max_workers=render_w) as render_ex:
                render_futs = {
                    render_ex.submit(_render_one, page_i, out_file, f, regions, dbg): page_i
                    for page_i, out_file, f, regions, dbg in render_items
                }
                for fut in render_futs:
                    try:
        ...
        self._package_output(processed_files, output_path, cache_dir, out_dir, debug_files)

    def _package_output(self, processed_files: List[str], output_path: str, cache_dir: str,
                         out_dir: str, debug_files: Optional[List[str]] = None) -> None:
    ...
    def apply_corrected_json(self, corrected_json_path: str, output_path: str) -> None:
        ...
        cache_dir = output_path + ".cache"
        out_dir = os.path.join(cache_dir, "out")
        regions_dir = os.path.join(cache_dir, "regions")
        if not os.path.isdir(regions_dir):
            raise FileNotFoundError(
                f"کش ناحیه‌ها پیدا نشد: {regions_dir}\n"
                "یعنی این خروجی یا با نسخه‌ای از اسکریپت ساخته شده که هنوز کش "
                "ناحیه‌ها رو ذخیره نمی‌کرد، یا کش قبلاً پاک شده (مثلاً چون بدون "
                "--keep-old دوباره روی همین خروجی اجرا شده)."
            )

        with open(corrected_json_path, "r", encoding="utf-8") as f:
            blocks = json.load(f)
        if not isinstance(blocks, list):
            raise ValueError("فایل JSON اصلاح‌شده باید آرایه‌ای از بلوک‌ها باشه.")

        corrected_lookup: Dict[Tuple[int, int], str] = {}
        for b in blocks:
            try:
                key = (int(b.get("page")), int(b.get("id")))
            except (TypeError, ValueError):
            ...
            t = (b.get("translation") or "").strip()
            if t:
                corrected_lookup[key] = t

        if not corrected_lookup:
            print("[!] هیچ دیالوگ معتبری توی JSON اصلاح‌شده پیدا نشد؛ کاری انجام نشد.",
                  file=sys.stderr)
            return

        page_cache_files = sorted(glob.glob(os.path.join(regions_dir, "page_*.json")))
        if not page_cache_files:
            print("[!] هیچ فایل کشی توی پوشهٔ regions پیدا نشد.", file=sys.stderr)
            return

        changed_pages = 0
        all_out_files: List[str] = []
        for cache_path in page_cache_files:
            try:
                with open(cache_path, "r", encoding="utf-8") as f:
                    page_data = json.load(f)
            except Exception as e:
                print(f"    [!] خواندن کش {os.path.basename(cache_path)} ناموفق: {e}")
                continue

            page_no = page_data.get("page")
            out_file = page_data.get("out_file")
            src_file = page_data.get("src_file")
            if not (page_no and out_file and src_file):
                continue
            all_out_files.append(out_file)

            regions = [self._region_from_dict(d) for d in page_data.get("regions", [])]
            page_changed = False
            for r in regions:
                new_text = corrected_lookup.get((page_no, r.id))
                if new_text and new_text != r.translated_text:
            ...
            if image is None:
                print(f"    [!] تصویر منبع صفحهٔ {page_no} پیدا نشد: {src_file}")
                continue

            try:
                result, _dbg = self.render_page_phase(image, regions, None)
            except Exception as e:
                print(f"    [!] رندر دوبارهٔ صفحهٔ {page_no} ناموفق: {e}", file=sys.stderr)
                continue

            self._write_image(result, out_file)

            
            page_data["regions"] = [self._region_to_dict(r) for r in regions]
            try:
                with open(cache_path, "w", encoding="utf-8") as f:
                    json.dump(page_data, f, ensure_ascii=False)
            except Exception as e:
                print(f"    [!] به‌روزرسانی کش صفحهٔ {page_no} ناموفق: {e}")

            changed_pages += 1
            print(f"    [✎] صفحهٔ {page_no} با متن اصلاح‌شده دوباره رندر شد: "
                  f"{os.path.basename(out_file)}")

        if changed_pages == 0:
            print("[*] هیچ دیالوگی نسبت به کش فعلی عوض نشده بود؛ فایل خروجی نهایی دست‌نخورده ماند.")
            return

        print(f"[*] {changed_pages} صفحه دوباره رندر شد (بدون OCR/ترجمهٔ دوباره)؛ "
              f"در حال بازسازی فایل خروجی نهایی...")
        self._package_output(sorted(all_out_files), output_path, cache_dir, out_dir)
        print(f"[✓] خروجی نهایی به‌روزرسانی شد: {output_path}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("-i", "--input", default=None,
                   help="مسیر ورودی. برای Pipeline کامل لازمه؛ برای --correct-json لازم نیست، "
                        "برای --apply-json هم لازم نیست.")
    p.add_argument("-o", "--output", default=None,
                   help="مسیر خروجی: پوشه، فایل کامل، یا فقط پسوند (.pdf / .zip / .html). "
                        "برای --apply-json باید همون مسیر خروجیِ اجرای قبلی باشه (برای پیدا "
                        "کردن پوشهٔ کش کنارش)؛ برای --correct-json لازم نیست.")
    p.add_argument("--correct-json", default=None, metavar="PATH",
                   help="حالت اصلاح فقط-متنی: به‌جای اجرای کامل Pipeline (OCR/ترجمه/رندر)، "
                        "فایل JSON کامل ترجمهٔ فصل (<output>_translations.json که خودِ "
                        "اسکریپت بعد از فاز ۴ می‌سازه) رو می‌گیره و بدون ارسال دوباره‌ی "
                        "تصویرها یا اجرای دوباره‌ی OCR، فقط با متن از AI می‌خواد کل فصل رو "
                        "یک‌بار دیگه بازبینی/اصلاح کنه؛ نسخهٔ اصلاح‌شده رو ذخیره می‌کنه "
                        "(پیش‌فرض: کنار همون فایل، با پسوند _corrected.json). در این حالت "
                        "--input/--output/--font لازم نیستن؛ فقط --api-key کافیه.")
    p.add_argument("--correct-output", default=None, metavar="PATH",
                   help="مسیر ذخیره‌ی JSON اصلاح‌شده (فقط با --correct-json).")
    p.add_argument("--apply-json", default=None, metavar="PATH",
                   help="فایل JSON اصلاح‌شده (خروجی --correct-json) رو می‌گیره و روی خروجی "
                        "موجود اعمال می‌کنه: فقط صفحه‌هایی که دیالوگشون واقعاً عوض شده، از "
                        "روی کش (بدون OCR/ترجمهٔ دوباره) دوباره رندر می‌شن و فایل خروجی نهایی "
                        "بازسازی می‌شه. --output باید همون مسیر خروجیِ اجرای اصلی باشه. "
                        "--input و --api-key لازم نیستن؛ --font لازمه (برای رندر دوباره).")
    p.add_argument(
        "--provider",
        default="gemini",
    ...
    p.add_argument("--font", default=None,
                   help="فونت پیش‌فرض فارسی / بالن عادی (کودک) و fallback. برای Pipeline "
                        "کامل و --apply-json لازمه؛ برای --correct-json لازم نیست.")
    ...
    p.add_argument("--bubbles-per-request", type=int, default=12,
                   help="چند حباب در هر درخواست ترجمه (پیش‌فرض ۱۲ — با توجه به منطق "
                        "تایم‌اوت سریع/تقسیم خودکار، بزرگ‌تر از قبل و مطمئن‌تره؛ برای "
                        "احتیاط بیشتر می‌تونی کمترش کنی)")
    ...
    p.add_argument("--no-proofread", action="store_true",
                   help="بازبینی/اصلاح یکپارچهٔ کل فصل بعد از ترجمه غیرفعال شود "
                        "(پیش‌فرض: فعال؛ فقط یک درخواست اضافه در هر فصل، بعد از ترجمهٔ "
                        "همهٔ صفحه‌ها و قبل از پاکسازی/رندر نهایی)")
...
def main():
    ...
    if args.correct_json:
        if not unique_keys and provider != "ollama":
            print(
                f"خطا: برای --correct-json هم حداقل یک کلید API لازمه "
                f"(--api-key یا env: {env_name}).",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        translator.correct_translations_from_json(args.correct_json, args.correct_output)
        return

    if args.apply_json:
        if not args.output:
            print("خطا: --apply-json به --output نیاز داره (همون مسیر خروجیِ اجرای اصلی).",
                  file=sys.stderr)
            sys.exit(1)
        if not args.font or not os.path.isfile(args.font):
            print("خطا: --apply-json برای رندر دوباره به یک فونت معتبر با --font نیاز داره.",
                  file=sys.stderr)
            sys.exit(1)
        translator = MangaTranslator(
            api_key=["ollama"],
        ...
        translator.apply_corrected_json(args.apply_json, args.output)
        return

    if not args.input or not args.output:
        print("خطا: برای اجرای کامل Pipeline، --input و --output لازمن "
              "(یا از --correct-json / --apply-json برای حالت‌های مستقل استفاده کن).",
              file=sys.stderr)
        sys.exit(1)
    if not args.font:
        print("خطا: برای اجرای کامل Pipeline، --font لازمه.", file=sys.stderr)
        sys.exit(1)
    ...
    translator = MangaTranslator(
        ...
        proofread=not getattr(args, "no_proofread", False),
    )
