...
def _expand_mask_boxes_local(mask_pil: "Image.Image", ratio: float = 0.10) -> "Image.Image":
    """
    Fallback for remove_object._expand_mask_boxes (that module is not part of
    this project, so the import above always fails and expansion was silently
    skipped). Grows every connected component of the mask by `ratio` of ITS
    OWN width/height (not a fixed global dilation), then fills the expanded
    rectangle solid. This keeps the growth proportional/adaptive per text
    block: small words get a small nudge, big stylized/decorative text blocks
    get a proportionally bigger margin, which is what LaMa needs to also take
    in glow/stroke/halo pixels that sit just outside the tight OCR polygon.
    """
    ...
    for i in range(1, n):
        ...
        if area < 2:
            continue
        pad_x = max(1, int(round(bw * ratio)))
        pad_y = max(1, int(round(bh * ratio)))
...
class SimpleLamaInpainter:
    ...
    def __call__(self, image, mask):
        ...
        if _lama_expand_mask_boxes is not None:
            ...
            try:
                mask_pil = _expand_mask_boxes_local(mask_pil, self.mask_expand_ratio)
            except Exception:
                pass
...
class MangaTranslator:
    ...
    def __init__(
        ...
        clean_promo_sfx: bool = True,
    ):
        
        
        
        self.clean_promo_sfx = bool(clean_promo_sfx)
    ...
    @staticmethod
    def _remove_wall_like_components(ink: np.ndarray, gray_crop: np.ndarray) -> np.ndarray:
        
        
        
        
        ch, cw = ink.shape[:2]
        if ch < 6 or cw < 6 or not np.any(ink):
            return ink
        try:
            edges = cv2.Canny(gray_crop, 50, 120)
            edges = cv2.dilate(edges, np.ones((3, 3), np.uint8), iterations=1)
            n, lab, st, _ = cv2.connectedComponentsWithStats(edges, connectivity=8)
            wall = np.zeros((ch, cw), dtype=np.uint8)
            for i in range(1, n):
                a = int(st[i, cv2.CC_STAT_AREA])
                bw = int(st[i, cv2.CC_STAT_WIDTH])
                bh = int(st[i, cv2.CC_STAT_HEIGHT])
                ...
                if ss <= 5 and ls >= max(18, int(0.5 * max(ch, cw))):
                    wall[lab == i] = 255
                elif a > 0.08 * ch * cw and ss <= 8:
                    wall[lab == i] = 255
            if not np.any(wall):
            ...
            return cv2.bitwise_and(ink, cv2.bitwise_not(wall))
        except Exception:
            return ink

    def _expand_region_mask_with_local_ink(
        self, image: np.ndarray, gray_full: np.ndarray, region: "TextRegion", region_mask: np.ndarray
    ) -> np.ndarray:
        ...
        try:
            ...
            margin_x = max(10, int(round(w * 0.35)))
            margin_y = max(10, int(round(h * 0.35)))
            x0 = max(0, x - margin_x)
            y0 = max(0, y - margin_y)
            ...
            ink = self._ink_mask_inside_bubble(gray_full, x0, y0, x1, y1)
            ...
            if zone is None or zone.shape != ink.shape:
                zone = region_mask[y0:y1, x0:x1]
                if zone.shape != ink.shape:
                    return region_mask

            
            near = cv2.dilate(
                zone, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11)), iterations=2
            )
            ...
            gray_crop = gray_full[y0:y1, x0:x1]
            ink = self._remove_wall_like_components(ink, gray_crop)
            if not np.any(ink):
                return region_mask

            expanded = region_mask.copy()
            expanded[y0:y1, x0:x1] = cv2.bitwise_or(expanded[y0:y1, x0:x1], ink)
            return expanded
        except Exception:
            return region_mask
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            det_class = (getattr(region, "det_class", "") or "").strip().lower()
            is_round_bubble = det_class in ("bubble", "text_bubble")
            if not is_round_bubble:
                region_mask = self._expand_region_mask_with_local_ink(image, gray_full, region, region_mask)
...
ترجمه ثبت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def _translate_regions_batch(self, regions: List[TextRegion]) -> None:
        ...
        user_prompt = (
            ...
            "متن‌ها دقیقاً همون چیزیه که OCR از تصویر خونده و ممکنه خراب، ناقص، چسبیده یا دارای غلط املایی باشه.\n"
            "فقط غلط‌های واضح حروف/تایپیِ همون کلمه رو در ذهنت اصلاح کن "
            "(مثلاً MUDIYING→MODIFYING، NDYE/AND YE→AND YET، RECONSTRUC→RECONSTRUCTION)؛ "
            "هیچ کلمه، جمله یا مفهومی که در متن OCR نیست را از context، بافت صحنه یا تصویر حدس نزن و اضافه نکن.\n"
            "سپس دقیقاً همون محتوایی که در متن OCR هست را به شکل یک دیالوگ طبیعی و روان فارسی بازنویسی کن؛ "
            "این یعنی فقط چینش و کلمه‌بندی جمله فارسی می‌شه، نه اینکه معنی یا محتوای تازه‌ای ساخته بشه.\n\n"
            f"{context_block}"
            "اصل مهم:\n"
            "ترجمه تحت‌اللفظی و کلمه‌به‌کلمه نکن؛ دیالوگ را طوری بنویس که انگار از اول به فارسی نوشته شده، "
            "ولی هیچ اطلاعات، حس یا جمله‌ای که در متن OCR نبوده اضافه نکن و چیزی از آن حذف نکن.\n"
            "اگر جمله در OCR ناقص/بریده است، در فارسی هم ناقص/بریده بماند؛ آن را کامل نکن.\n"
    ...
    def finish_page_phase(self, image: np.ndarray, regions: List[TextRegion],
        ...
        if self.clean_promo_sfx:
            if promo_regions:
                print(f"  [*] {len(promo_regions)} تبلیغ → پاک می‌شود (بدون رندر فارسی)")
            if sfx_regions:
                print(f"  [*] {len(sfx_regions)} SFX → پاک می‌شود (بدون رندر فارسی)")
        else:
            if promo_regions:
        ...
        regions_to_clean = list(translated_regions)
        if self.clean_promo_sfx:
            regions_to_clean += promo_regions + sfx_regions

        print("[فاز ۴ - پاکسازی متن + رندر] ...")
        if regions_to_clean:
            cleaned_image = self.clean_image(image, regions_to_clean)
