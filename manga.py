...
ترجمه ثبت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def _generate_proofread_json(self, system_instruction: str, user_prompt: str):
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                with self._api_lock:
                    if self.provider_type == "gemini":
                        text = self._proofread_with_gemini(user_prompt, system_instruction)
                    else:
                        text = self._proofread_with_openai(user_prompt, system_instruction)
                cleaned = text.strip()
                ...
                if not isinstance(result, list):
                    raise ValueError("پاسخ بازبینی آرایه نیست.")
                ...
                if self.provider_type == "gemini" and self._is_daily_quota_error(e):
                    raise GeminiQuotaExhausted(
                        f"سهمیهٔ روزانهٔ {self.model_name} تمام شد؛ بازبینی فصل متوقف شد."
                    ) from e
                ...
                if self._is_context_too_large_error(e):
                    print(f"    [!] این Request بزرگ‌تر از Context Window مدل «{self.model_name}» "
                          f"است (نه خطای موقت) → تلاش مجدد بی‌فایده‌ست، این بخش نصف می‌شود.")
                    return None

                if self._is_model_permanently_gone(e):
                    if self._drop_current_model_and_switch(reason="proofread model gone"):
                        self._recreate_api_client()
                        time.sleep(0.3)
                        continue
                if self._is_model_unavailable_error(e):
                    if self._switch_to_next_model(reason="proofread unavailable"):
                        time.sleep(0.2)
                        continue
                    if self._switch_to_next_key(reason="proofread model unavailable", cycle=True):
                        self._recreate_api_client()
                        time.sleep(0.3)
                        continue
                ...
                if self._is_rate_or_model_quota_error(e) or any(
                    ...
                    if self._switch_to_next_model(reason="proofread rate/quota"):
                        time.sleep(0.5)
                        continue
                    if self._switch_to_next_key(reason="proofread rate/quota", cycle=True):
                        self._recreate_api_client()
                        time.sleep(1.0)
                        continue
                ...
                print(f"    [!] بازبینی: تلاش {attempt}/{self.max_retries} ناموفق (خطای موقت) — "
                      f"همین یک Request کل‌فصل دوباره امتحان می‌شه: {last_err}")
                if attempt < self.max_retries:
                    time.sleep(delay)
                    delay = min(delay * 1.6, 8.0)

        print(f"    [!] بازبینی بعد از {self.max_retries} تلاش روی همین یک Request هم ناموفق موند "
    ...
    def finish_page_phase(self, image: np.ndarray, regions: List[TextRegion],
        ...
        dialogue_regions = self.translate_page_phase(regions)

        if self.proofread_enabled and dialogue_regions:
            proof_regions = [r for r in dialogue_regions if (r.translated_text or "").strip()]
            if proof_regions:
                print(f"[فاز ۴ - بازبینی] {len(proof_regions)} دیالوگ این صفحه ...")
                try:
                    self.proofread_chapter([(1, proof_regions)])
                except GeminiQuotaExhausted:
                    raise
                except Exception as e:
                    print(f"    [!] خطا در بازبینی: {e}", file=sys.stderr)
...
"""
    ...
    def run(self, input_path: str, output_path: str, resume: bool = True,
        ...
        def _translate_one(page_i, out_file, f, image, regions, dbg):
            
            if image is None:
                return page_i, out_file, f, None, dbg
            if not regions:
                return page_i, out_file, f, regions, dbg
            try:
                self.translate_page_phase(regions)
            except GeminiQuotaExhausted:
                raise
            except Exception as e:
                print(f"    [!] خطا در ترجمه‌ی {os.path.basename(out_file)}: {e}", file=sys.stderr)
            return page_i, out_file, f, regions, dbg
        ...
        translated_by_i: Dict[int, tuple] = {}

        if len(pending) <= 1:
            for item in pending:
                try:
                    page_i, out_file, f, image, regions, dbg = _extract_one(item)
                    page_i, out_file, f, regions, dbg = _translate_one(
                        page_i, out_file, f, image, regions, dbg)
                    del image
                except GeminiQuotaExhausted as e:
                    print(f"\n[!] {e}")
                    break
                translated_by_i[page_i] = (out_file, f, regions, dbg)
        else:
            from concurrent.futures import FIRST_COMPLETED, wait as fut_wait
            from collections import deque
            print(
                f"[*] استخراج دوطرفه: اول از بالا→پایین + آخر از پایین→بالا | "
                f"ترجمه موازی ({len(pending)} صفحه)"
            )
            extract_w = 2  
                    ...
                    for fut in done:
                        if fut in extract_futs:
                            ...
                            if image is not None:
                                pf = post_ex.submit(
                                    _translate_one, page_i, out_file, f, image, regions, dbg
                                )
                                post_futs[pf] = page_i
                            else:
                                translated_by_i[page_i] = (out_file, f, regions, dbg)
                            ...
                            try:
                                page_i, out_file, f, regions, dbg = fut.result()
                            except GeminiQuotaExhausted as e:
                                print(f"\n[!] {e}")
                                api_dead[0] = True
                                continue
