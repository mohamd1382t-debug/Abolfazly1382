...
# Lexicons of *meaningful* English words. They are used by _classify_text() and
# _is_sfx_token(): a token that is a real word (OK, STOP, RIGHT ...) is never
# treated as an SFX just because it is short or upper-case.
DIALOGUE_SHORT_WORDS = frozenset({
...
class MangaTranslator:
    ...
    @staticmethod
    def _is_sfx_token(text: str, det_class: str = "") -> bool:
        """True only for a lone onomatopoeia-like token (GAW, GRRR, KYAAA, BOOM ...).

        Decided on evidence, never on "short + UPPER-CASE" alone:
          * a real/meaningful word (OK, STOP, RIGHT ...) is never SFX by text alone;
          * inside a speech bubble (det_class text_bubble/bubble) only the strictest
            evidence (no vowel at all: HNGH, TSK, PFFT) counts - so bubble dialogue is
            classified exactly as before;
          * outside a bubble also: stretched letters (KYAAA, HAAA, ZZZ) and the curated
            onomatopoeia in SFX_WORD_RE (BOOM, CRASH, THUD ...);
          * only for detector "text_free" text (art-integrated, outside any bubble) also
            a 3-4 letter ALL-CAPS single-syllable token that is not a known word
            (GAW, KRAK ...).
        Repeated tokens ("GAW GAW", "BOOM! BOOM!") are judged by the repeated token.
        """
        s = (text or "").strip()
        if not s:
            return False
        ...
        parts = s.split()
        if len(parts) > 1:
            norm = {re.sub(r"[!?.…~]+$", "", p).lower() for p in parts}
            if len(parts) > 4 or len(norm) != 1:
                return False
            s = parts[0]

        m = re.fullmatch(r"([A-Za-z]{2,12})[!?.…~]*", s)
        if not m:
            return False
        word = m.group(1)
        low = word.lower()
        squeezed = re.sub(r"(.)\1{2,}", r"\1", low)

        if (
            low in DIALOGUE_SHORT_WORDS
            or squeezed in DIALOGUE_SHORT_WORDS
            or word.upper() in COMMON_UPPER_WORDS
        ):
            return False

        if len(word) >= 3 and not re.search(r"[aeiouy]", low):
            return True

        if (det_class or "").lower() in ("text_bubble", "bubble"):
            return False

        if re.search(r"(.)\1{2,}", low):
            return True
        if SFX_WORD_RE.match(word):
            return True
        # GAW, KRAK, THUD ... : a single-syllable burst (exactly one vowel group), 3-4 letters.
        # Multi-syllable words (HELLO, ENOUGH, MONEY, TABLE) can never match this.
        if (
            (det_class or "").lower() == "text_free"
            and word.isupper()
            and 3 <= len(word) <= 4
            and len(re.findall(r"[aeiouy]+", low)) == 1
        ):
            return True
        return False

    @staticmethod
    def _classify_text(text: str, det_class: str = "") -> str:

        stripped = (text or "").strip()
        if not stripped:
            return "junk"

        # SFX must be judged BEFORE the generic "Latin letters + a vowel => dialogue"
        # shortcuts below; otherwise GAW / BOOM / KRAK are turned into dialogue here and
        # never reach any SFX check. The verdict is applied further down (after the
        # promo/watermark checks, which keep priority).
        sfx_like = MangaTranslator._is_sfx_token(stripped, det_class)

        
        latin_core = re.sub(r"[^A-Za-z]", "", stripped)
        if not sfx_like and len(latin_core) >= 3 and re.fullmatch(r"[A-Za-z][A-Za-z\s.'\-]*[.!?…~]*", stripped.replace("...", ".").replace("…", ".")):
            
            if any(c in "AEIOUaeiou" for c in latin_core):
                return "dialogue"
        if not sfx_like and len(latin_core) >= 2 and stripped.endswith(("?", "!", "?!", "!?", "...?", "...!")):
            if any(c in "AEIOUaeiou" for c in latin_core):
                return "dialogue"
        ...
        dialogue_short = DIALOGUE_SHORT_WORDS
        ...
        if (
            ...
            _common_upper = COMMON_UPPER_WORDS
...
ترجمه ثبت بشه؛ در غیر این صورت آرایه رو خالی بذار."""
    ...
    def _extract_regions_from_bubbles(self, image: np.ndarray) -> List[TextRegion]:
        ...
        for i, b in enumerate(boxes):
            ...
            kind = self._classify_text(text, b.get("class_name", "") or "")
