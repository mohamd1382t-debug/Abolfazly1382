...
"""
        ...
        with gr.Row():
            ...
            stop_btn = gr.Button("⏹  توقف ترجمه", variant="stop", elem_id="stopbtn")
...
"""
        ...
        def run_translation(sid, sid_box_v, inp_path_v, upload, provider_v, api_keys_v, model_v,
            ...
            if running_now:
                # یک ترجمه از قبل روی همین session در حال اجراست.
                # به‌جای کشتنِ ناخواسته‌ی job (که قبلاً همین دکمه انجام می‌داد و باعث
                # قطع‌شدن وسط ترجمه بعد از رفرش/قطعی موقت می‌شد)، فقط وضعیت فعلی را
                # نشان می‌دهیم. برای توقف واقعی باید از دکمه‌ی «⏹ توقف ترجمه» استفاده شود.
                msg = job.get("log") or "▶ یک ترجمه از قبل در حال اجراست…"
                return _pack("⏹  متوقف ترجمه", msg)
        ...
        def stop_translation(sid, sid_box_v):
            def _pack_stop(btn, log):
                return (
                    sid,
                    gr.update(value=sid),
                    gr.update(value=btn),
            ...
            sid = _find_active_sid(sid) or sid
            if not sid:
                return _pack_stop("🚀  شروع ترجمه", "چیزی برای متوقف کردن وجود ندارد.")
            job = _get_job(sid)
            with job["lock"]:
                running_now = _job_running(job)
            if not running_now:
                msg = job.get("log") or "چیزی در حال اجرا نیست."
                return _pack_stop("🚀  شروع ترجمه", msg)
            _kill_job(sid)
            msg = job.get("log") or "⏹ ترجمه متوقف شد توسط کاربر."
            return _pack_stop("🚀  شروع ترجمه", msg)

        _stop_confirm_js = """
() => {
  if (!confirm("مطمئنی می‌خوای ترجمه رو متوقف کنی؟ این کار قابل بازگشت نیست و باید از اول شروع کنی.")) {
    throw new Error("توقف لغو شد توسط کاربر.");
  }
  return [];
}
"""
        ...
        try:
            stop_btn.click(stop_translation, js=_stop_confirm_js,
                            show_progress="hidden", **_stop_click_kw)
        except TypeError:
            try:
                stop_btn.click(stop_translation, js=_stop_confirm_js,
                                show_progress=False, **_stop_click_kw)
            except TypeError:
                stop_btn.click(stop_translation, js=_stop_confirm_js, **_stop_click_kw)
