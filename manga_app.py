...
FONT_BUNDLES = [
    ("normal",       "Far_KoodkBd.ttf", "کودک — متن عادی حباب", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("free_text",    "Mj-Tehran.ttf", "تهران — متن بیرون حباب", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("shout",        "AFSANEH.ttf", "افسانه — داد خشم", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("comedy_shout", "B_Kourosh_0.ttf", "کروش — داد کمدی", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("whisper",      "Far_DastNevis.otf", "دست‌نویس — زمزمه", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("thought",      "Far_Morvarid.ttf", "مروارید — تفکر ابری", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
    ("system",       "Far_Farnaz.ttf", "فرناز — UI سیستم/تگ", [
        # فونت محلی جایگزین‌شده توسط کاربر — نیازی به دانلود نیست
    ]),
...
def find_font() -> str:
    cands = []
    normal_fname = _normal_font_filename()
    if os.path.isdir(FONT_DIR):
        for f in sorted(os.listdir(FONT_DIR)):
            if f.lower().endswith((".ttf", ".otf")):
                cands.append((0 if f.lower() == normal_fname else 1, os.path.join(FONT_DIR, f)))
...
"""
        ...
        def run_translation(sid, sid_box_v, inp_path_v, upload, provider_v, api_keys_v, model_v,
            ...
            cmd += font_args()
            cli_font_tone = {"free_text": "free"}
            for slot, fp in tone_map.items():
                if fp and os.path.isfile(fp):
                    cmd += ["--font-" + cli_font_tone.get(slot, slot.replace("_", "-")), fp]
